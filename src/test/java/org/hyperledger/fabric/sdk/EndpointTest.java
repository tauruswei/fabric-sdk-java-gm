...
public class EndpointTest {
    ...
    public void testEndpointNonPEM() {
        Endpoint ep = new Endpoint(hfclient.getCryptoSuite(), "grpc://localhost:524", null);
        Assert.assertEquals("localhost", ep.getHost());
        Assert.assertEquals(524, ep.getPort());

        ep = new Endpoint(hfclient.getCryptoSuite(), "grpcs://localhost:524", null);
        Assert.assertEquals("localhost", ep.getHost());

        try {
            new Endpoint(hfclient.getCryptoSuite(), "grpcs2://localhost:524", null);
        ...
        try {
            new Endpoint(hfclient.getCryptoSuite(), "grpcs://localhost", null);
            Assert.fail("should have thrown error as there is no port in the url");
        } catch (RuntimeException rex) {
            Assert.assertEquals("URL must be of the format protocol://host:port. Found: 'grpcs://localhost'",
                rex.getMessage());
        }

        try {
            new Endpoint(hfclient.getCryptoSuite(), "", null);
        ...
        try {
            new Endpoint(hfclient.getCryptoSuite(), null, null);
            Assert.fail("should have thrown error as url is empty");
        } catch (RuntimeException rex) {
            Assert.assertEquals("URL cannot be null or empty", rex.getMessage());
        }
    ...
    public void testNullPropertySslProvider() {
        ...
        new Endpoint(hfclient.getCryptoSuite(), "grpcs://localhost:594", testprops);
    }
    ...
    public void testExtractCommonName() {
        ...
        Assert.assertSame(new Endpoint(hfclient.getCryptoSuite(), "grpcs://localhost:594", testprops).getClass(),
            Endpoint.class);
    }
    ...
    public void testClientTLSInvalidProperties() {
        ...
        try {
            ...
            Assert.assertEquals("Properties \"clientKeyFile\" and \"clientKeyBytes\" must cannot both be set",
                e.getMessage());
        }
        ...
        try {
            ...
            Assert.assertEquals("Properties \"clientCertFile\" and \"clientCertBytes\" must cannot both be set",
                e.getMessage());
        }
        ...
        try {
            ...
            Assert.assertTrue(
                e.getMessage().contains("Failed endpoint grpcs://localhost:594 to parse TLS client private key"));
        }
    }
    ...
    public void testClientTLSProperties() {
        ...
        testprops.setProperty("clientKeyFile", System.getProperty("user.dir") + "/src/test/resources/tls-client.key");
        testprops.setProperty("clientCertFile", System.getProperty("user.dir") + "/src/test/resources/tls-client.crt");
        Endpoint endpoint = new Endpoint(hfclient.getCryptoSuite(), "grpcs://localhost:594", testprops);
    ...
    public void testClientTLSCACertProperties() throws Exception {
        ...
        testprops.put("pemBytes",
            Files.readAllBytes(Paths.get("src/test/fixture/testPems/Org2MSP_CA.pem"))); //Can have pem bytes too. 1 cert

        class TEndpoint extends Endpoint {

            private SslContextBuilder sslContextBuilder;

            TEndpoint(CryptoSuite cs, String url, Properties properties) {
                super(cs, url, properties);
            }

            @Override
            protected SslContextBuilder getSslContextBuilder(X509Certificate[] clientCert, PrivateKey clientKey,
        ...
        TEndpoint endpoint = new TEndpoint(hfclient.getCryptoSuite(), "grpcs://localhost:594", testprops);
        ...
        assertEquals("Didn't find the expected number of certs", expected.size(),
            certs.length); // should have same number.
    }
